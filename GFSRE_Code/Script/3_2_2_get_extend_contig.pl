...
system("/home/ktwang/Code/cap3/CAP3/cap3 5end_match_reads > 5end_match_reads.out");
@array = `cat 3end_match_reads`;
if(@array>2000){		# only keep 1000 reads
	`cp 3end_match_reads 3end_match_reads2`;
	`head -2000 3end_match_reads2 > 3end_match_reads`;
}
system("/home/ktwang/Code/cap3/CAP3/cap3 3end_match_reads > 3end_match_reads.out");
...
foreach my $tag(@tags){
	...
	if($contig_num == 0){		# 1. no contig was same as index seq after firstly assembly
		...
		system("/home/ktwang/Code/cap3/CAP3/cap3 $tag\_match_reads.cap.contigs > $tag\_match_reads.cap.contigs.out");	# 第二次组装
		system("rm *ace *links *qual *info *singlets");
